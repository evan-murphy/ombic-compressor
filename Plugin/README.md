...
JUCE 7+ VST3 plugin with compressor mode selection (Opto / FET), neon saturation block, and saturator always before compressor. GUI follows the [OMBIC Sound Style Guide](../style/OMBIC_SOUND_STYLE_GUIDE.md).
...
- **Compressor**: Measured curves from the analyzer (`output/fetish_v2/`, `output/lala_v2/`). FET mode uses threshold (dB), ratio, attack/release with envelope smoothing from `timing.csv`. Opto uses threshold 0–100 with a gentler curve; gain reduction is scaled so it can sound more aggressive.
