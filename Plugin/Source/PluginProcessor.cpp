...
namespace
{
/** Find .../Resources/CurveData next to the running module (VST3 bundle, AU component, app, etc.). */
juce::File getBundledCurveDataRoot()
{
    ...
    {
        juce::File curveData = dir.getChildFile("Resources/CurveData");
...
void OmbicCompressorProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    monoStereoWork_.setSize(2, juce::jmax(512, samplesPerBlock));
}
...
bool OmbicCompressorProcessor::isBusesLayoutSupported(const juce::BusesLayout& layouts) const
{
    const auto inMain = layouts.getChannelSet(true, 0);
    const auto outMain = layouts.getChannelSet(false, 0);
    if (inMain.isDisabled() || outMain.isDisabled())
        return false;
    if (inMain != outMain)
        return false;
    if (inMain != juce::AudioChannelSet::mono() && inMain != juce::AudioChannelSet::stereo())
        return false;
    if (layouts.inputBuses.size() > 1)
    {
        const auto inSc = layouts.getChannelSet(true, 1);
        if (!inSc.isDisabled()
            && inSc != juce::AudioChannelSet::mono()
            && inSc != juce::AudioChannelSet::stereo())
            return false;
    }
    return true;
}

bool OmbicCompressorProcessor::supportsDoublePrecisionProcessing() const
{
    return true;
}
...
void OmbicCompressorProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    juce::ScopedNoDenormals noDenormals;
    auto mainOut = getBusBuffer(buffer, false, 0);
    const int numSamples = mainOut.getNumSamples();
    const int numChannels = mainOut.getNumChannels();
    if (numChannels == 0 || numSamples == 0)
        return;

    const juce::AudioBuffer<float>* extSc = nullptr;
    juce::AudioBuffer<float> scBuf;
    if (getBusCount(true) > 1)
    {
        if (auto* bus = getBus(true, 1))
        {
            if (bus->isEnabled() && !bus->getCurrentLayout().isDisabled())
            {
                scBuf = getBusBuffer(buffer, true, 1);
                if (scBuf.getNumChannels() > 0)
                    extSc = &scBuf;
            }
        }
    }
    ...
    if (numChannels == 1)
    {
        monoStereoWork_.setSize(2, numSamples, false, false, true);
        monoStereoWork_.copyFrom(0, 0, getBusBuffer(buffer, true, 0), 0, 0, numSamples);
        monoStereoWork_.copyFrom(1, 0, getBusBuffer(buffer, true, 0), 0, 0, numSamples);
        processMainBusFloat(monoStereoWork_, extSc, midi);
        mainOut.copyFrom(0, 0, monoStereoWork_, 0, 0, numSamples);
        return;
    }

    processMainBusFloat(mainOut, extSc, midi);
}

void OmbicCompressorProcessor::processBlock(juce::AudioBuffer<double>& buffer, juce::MidiBuffer& midi)
{
    juce::ScopedNoDenormals noDenormals;
    ...
    floatScratchForDouble_.setSize(nCh, nS, false, false, true);
    for (int c = 0; c < nCh; ++c)
        for (int i = 0; i < nS; ++i)
            floatScratchForDouble_.setSample(c, i, static_cast<float>(buffer.getSample(c, i)));

    processBlock(floatScratchForDouble_, midi);

    for (int c = 0; c < nCh; ++c)
        for (int i = 0; i < nS; ++i)
            buffer.setSample(c, i, static_cast<double>(floatScratchForDouble_.getSample(c, i)));
}

void OmbicCompressorProcessor::processMainBusFloat(juce::AudioBuffer<float>& buffer,
                                                   ...
                                                   juce::MidiBuffer&)
{
    ...
    // Sidechain filter: mono sum of detector path (external sidechain bus if connected, else main input)
    // All getRawParameterValue() return normalized 0..1; convert to actual range for processing.
    const juce::AudioBuffer<float>& detectorSource =
        (externalSidechain != nullptr && externalSidechain->getNumChannels() > 0)
            ? *externalSidechain
            : buffer;
    ...
    const int detCh = detectorSource.getNumChannels();
    for (int i = 0; i < numSamples; ++i)
    {
        float sum = 0.0f;
        for (int ch = 0; ch < detCh; ++ch)
            sum += detectorSource.getSample(ch, i);
        mono[i] = sum / static_cast<float>(juce::jmax(1, detCh));
    }
